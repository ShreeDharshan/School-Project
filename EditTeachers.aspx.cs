...
namespace CompSciIA
{
    public partial class EditTeachers : System.Web.UI.Page
    {
        ...
        private void BindGrid()
        {
            ...
            {
                using (SqlCommand cmd = new SqlCommand("SELECT Teacher, Grade FROM TeacherGrade "))
                {
        ...
        {
            {
                ...
                string teachername = (row.Cells[0].Controls[0] as TextBox).Text;
                ...
                using (SqlConnection con = new SqlConnection(connectionString))
                {
                    using (SqlCommand cmd = new SqlCommand("UPDATE TeacherGrade SET Grade = @Grade where Teacher ='"+teachername+"'"))
